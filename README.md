...
After installing Product Stock History, your products will get a new meta box on their editing pages, where you will see a history of when the stock/quantity changed for that product. In most cases, it shows you what caused it (order placed, manual edit, etc.).

**What if I have a Variable Product?**  
For Variable Products, the box will have a dropdown where you can select any variation to see its own stock history. Note that it's possible to keep track of inventory for one variation, but not the other! In that case, the variation that doesn't have any tracking will follow the stock quantity of the parent product.

**Come again?**
- If a variation has its own stock tracking, you can see its history by selecting that variation from the dropdown.
...
No, the tracking will only start once the plugin is installed. It cannot generate a history retroactively.
...
Product Stock History's community support forum at https://wordpress.org/support/plugin/product-stock-history-for-woocommerce would be a good place, though if you want to add all sorts of technical details, it's best to report it on the plugin's [GitHub issues page](https://github.com/senff/WooCommerce-Product-Stock-History/issues). This is also where code contributions are considered.

**My question isn't listed here?**  
Please go to the plugin's [community support forum](https://wordpress.org/support/plugin/product-stock-history-for-woocommerce) and post a message. Note that support is provided on a voluntary basis and that it can be difficult to troubleshoot, and may require access to your admin area. Needless to say, NEVER include any passwords of your site on a public forum!
